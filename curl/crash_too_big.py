...
class Browser:
        ...
        def __init__(self, input_file):
                ...
                self.profile = FirefoxProfile("/Users/wiki/Library/Application Support/Firefox/Profiles/3ig8qcrl.default-1435148476151")
                self.profile.set_preference("toolkit.startup.max_resumed_crashes", "-1")
                ...
                self.driver.get("http://www.lulu.com/author/index.php")#change to author page
        ...
        def loginPage(self):
                ...
                r_uid.send_keys(self.lulu_email)
        ...
        def makingYour(self):
                ...
                self.execution(self.next_disable,85,"click")
                print "let our greasey monkey screw the wizard"
                self.stage+=1
                self.luluCruise()
        def uploadCover(self):
                print "going to get fcid."
                cur_url = self.driver.current_url 
                self.fCID= cur_url.split("=")[1]
                print "k going to check this other shit now."
                is_disabled = self.execution(self.next_disable,10,"find")
                ...
                self.luluCruise()
        def extra(self):
                self.execution(self.next,45,"click")
                self.stage+=1
                self.luluCruise()
        def pubOptions(self):
        ...
        def selectFromProj(self):
                self.loginPage()
                self.search_proj = '//*[@id="searchToggle"]'
                self.search_title = '//*[@id="SearchTitleField"]'
                self.submit_search = '//*[@id="searchMyProjectsFormSubmit"]'
                self.execution(self.search_proj,20,"click")
                st = self.execution(self.search_title,20,"text")
                st.send_keys("Volume " + str(self.volume.num))
                self.execution(self.submit_search,20,"click")
                sleep(1)
                is_success = self.execution('//*[@id="noProjectsToList"]/h3',3,"find")
                if is_success==True:
                    self.changeUser()
                self.execution("/html/body/div[1]/div[1]/div[2]/div/div[2]/div[2]/div[2]/table/tbody/tr[1]/td[4]/a",40,"click")
                self.stage=8
                self.luluCruise()

        def changeUser(self):
                if self.lulu_email == "printwikipedia@printwikipedia.com":
                    self.lulu_email = "printwikipedia+1@printwikipedia.com"
                elif self.lulu_email == "printwikipedia+1@printwikipedia.com":
                    self.lulu_email = "printwikipedia+2@printwikipedia.com"
                elif self.lulu_email == "printwikipedia+2@printwikipedia.com":
                    self.lulu_email = "printwikipedia+3@printwikipedia.com"
                elif self.lulu_email == "printwikipedia+3@printwikipedia.com":
                    self.lulu_email = "printwikipedia+4@printwikipedia.com"
                elif self.lulu_email == "printwikipedia+4@printwikipedia.com":
                    self.lulu_email = "printwikipedia+5@printwikipedia.com"
                elif self.lulu_email == "printwikipedia+5@printwikipedia.com":
                    self.lulu_email = "printwikipedia+6@printwikipedia.com"
                else:
                    print "sorry figure this shit out."
                    exit()
                self.driver.get("http://www.lulu.com/account/logout")
                sleep(1)
                self.driver.get("http://www.lulu.com/author/index.php")#change to author page
                self.stage=13
                self.luluCruise()
...
class Local:
        ...
        def __init__(self, volume):
                ...
                def getFolders():#sets these vars.
                        ...
                        self.cFolder = self.pwd+"/ps_covers"
                getFolders()
        def immigration(self,volume,driver,i_fail=False,err_type=""):#move file from one folder to another. give next file over. This is done so there isn't such a long read for the 6k file run through.
                ...
                if i_fail==True:
                        ...
                        print json_s
                        json_f = open("7-15-goodboy.json","a")
                        json_f.write(json_s)
                        json_f.close()
        ...
        def ftpIt(self,file_name=False):
                if file_name is True:
                    ...
                    try:
                       session = ftplib.FTP(ftp_host,self.lulu_email,lulu_pass)
                       ...
                       if os.path.isfile("/Users/wiki/repos/printwikipedia/curl/redistilled-helvs"+'/'+input_file):
                           file = open("/Users/wiki/repos/printwikipedia/curl/redistilled-helvs"+'/'+input_file,'rb') # file to send
                       else:
                           file = open(self.inFolder+self.volume.round_folder+'/'+self.volume.input_file,'rb') # file to send
